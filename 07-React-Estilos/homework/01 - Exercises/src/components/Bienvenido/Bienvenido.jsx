...
//LO HABIA EMPEZADO A HACER PERO NO ERA ESTE

export default function Bienvenido () {
  return (
    <div>
      <h1 className={}>soy Henry!</h1>
      <h3 className="title">{studentName}</h3>
      <ul className="unorderedList">
        {techSkills.map(skill => (
          <li className="listItem" key={skill}>{skill.tech}<img className="listItem img" src={skill.image} alt={skill.tech} /></li>
        ))}
      </ul>
